  ...
  return (
    <section ref={aboutRef} id="about" className="py-32 relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {/* Tech Stack with Flip Animation */}
        <div className="mt-32">
            ...
            {techStackData.map((tech, index) => (
              ...
              >
                <div className="relative preserve-3d group-hover:rotate-y-180 transition-transform duration-700">
                  {/* Front */}
                  <div className="backface-hidden flex flex-col items-center space-y-4 p-6 bg-gray-800/50 rounded-lg border border-gray-700/50 hover:border-purple-500/30 transition-all duration-300">
                    <div className="text-4xl">{tech.icon}</div>
                    <span className="text-sm font-light text-gray-400 text-center">{tech.name}</span>
                  </div>

                  {/* Back */}
                  <div className="absolute inset-0 backface-hidden rotate-y-180 flex flex-col items-center justify-center p-6 bg-gradient-to-br from-purple-500/20 to-red-500/20 rounded-lg border border-purple-500/30">
                    <div className="text-2xl text-white mb-2">{tech.icon}</div>
                    <span className="text-xs font-light text-white text-center">{tech.name}</span>
                    <div className="w-8 h-0.5 bg-gradient-to-r from-purple-400 to-red-400 mt-2" />
                  </div>
                </div>
              </div>
            ))}
